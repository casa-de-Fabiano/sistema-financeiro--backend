...
export async function atualizarDespesas(id, despesas) {
  const comando = ` 
      UPDATE tb_despesas
      SET dt_referencia = ?, ds_tipo = ?, vl_despesas = ?, nr_parcelas = ?, dt_despesas = ?, ds_observacao = ?, file_recibo = ?, id_conta_origem = ?, id_conta_destino = ? WHERE id_despesas = ?  `;
  const resposta = await con.query(comando, [despesas.referencia, despesas.tipo, despesas.valorDespesa, despesas.parcelas, despesas.dt_despesas, despesas.observacao, despesas.recibo, despesas.idContaOrigem, despesas.idContaDestino, id]);
